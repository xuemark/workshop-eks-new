...
### 2. check pod status
There are 7 pod in Pending status.
```
kubectl get pod
...
```
eksctl scale nodegroup --cluster=${CLUSTER_NAME} --nodes=2 --name=${NODEGROUP_NAME} --region=${AWS_REGION}
...
```
### 11. stop load
- shut down ab test, pods will scale in to 1.
