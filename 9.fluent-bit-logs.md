...
### check s3 file
```
aws s3 ls s3://${S3_BUCKET}/fluentbit-logs/ --recursive
```
...
echo $S3_BUCKET
```
- 在左上角Query editor中，在New query中，输入下面的语句创建test_logs表，替换下面的BUCKET_NAMEs
```
CREATE EXTERNAL TABLE `test_logs`(
   `date` string, 
   `timestamp` string, 
   `no` string, 
   `pod_name` string)
