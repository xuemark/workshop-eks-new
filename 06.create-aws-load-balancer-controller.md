...
### create elb-iam-policy.json
```
cat <<EOF > elb-iam-policy.json
{
    "Version": "2012-10-17",
    "Statement": [
        {
...
aws iam create-policy --policy-name AWSLoadBalancerControllerIAMPolicy \
  --policy-document file://./elb-iam-policy.json --region ${AWS_REGION}
```
### export ENV 
```
...
```
kubectl apply -f controller-2.1.3.yaml
```
### view pod
```
kubectl get pod -A
...
## 6. create ingress for service - create ALB

### add subnet tags
```
subnets=$(aws ec2 describe-subnets  --filters Name=tag:Name,Values="Public Subnet 1","Public Subnet 2","Public Subnet 3" --region $AWS_REGION --query "Subnets[*].{SubnetId:SubnetId}" --o text)

aws ec2 create-tags --resources ${subnets} --region ${AWS_REGION} --tags Key=kubernetes.io/cluster/${CLUSTER_NAME},Value=shared

aws ec2 create-tags --resources ${subnets} --region ${AWS_REGION} --tags Key=kubernetes.io/role/elb,Value=1
